...
VERSION='showfits.py V1.0 (2021-03-16) (c) USM, Arno Riffeser (arri@usm.lmu.de)'
...
class Image_Analyzer:
  ...
  def __init__(self, fitradius=7, precaper=80, maxaper=80, zoom=0, verbose=2, divvy=False, outfile=None) :
    ...
    self.cmap = None
    self.cmap_nr = 0
    self.cmap_all = ['gist_heat','gist_heat_r','gist_gray','gist_gray_r','gist_ncar','gist_ncar_r','gist_rainbow',
                     'gist_rainbow_r','gist_stern','gist_stern_r','gist_yarg','gist_yarg_r','gist_earth','gist_earth_r']
  ...
  def start(self, fig) :
    ...
    self.ax      = self.fig.add_axes([0.07, 0.07, 0.91, 0.91]) 
    self.axnext  = self.fig.add_axes([0.12, 0.01, 0.10, 0.03])
    self.axcut0n = self.fig.add_axes([0.23, 0.01, 0.06, 0.03])
    self.axcut0p = self.fig.add_axes([0.30, 0.01, 0.06, 0.03])
    self.axcut1n = self.fig.add_axes([0.37, 0.01, 0.06, 0.03])
    self.axcut1p = self.fig.add_axes([0.44, 0.01, 0.06, 0.03])
    self.axcmap  = self.fig.add_axes([0.51, 0.01, 0.06, 0.03])
  ...
  def fit_psf(self, pos, use_moffat=False, verbose=0) :
    ...
    if not use_moffat : ########### GAUSS ###########
      ...
      if (not np.isnan(afit[0]) and not np.isnan(afit[1]) and
          afit[0]>x0 and afit[0]<x1 and afit[1]>y0 and afit[1]<y1 and afit[5]>afit[6]) :
  ...
  def growing_curve_close(self, event_buf) :
    print('---------------------------------------------------------------------------------------')
  ...
  def onclick(self, event):
    ...
    if (self.verbose>=5) :
      print('%s click: button=%s, key=%s, x=%d, y=%d, xdata=%f, ydata=%f' %
    ...
    if (event.button==1) :
      ...
      if (event.key=='1')  :
        ...
        if (not np.isnan(self.xc) and not np.isnan(self.yc) ) :
          ...
          self.id2_button_press = self.fig2.canvas.mpl_connect('button_press_event',self.growing_curve)
          self.id2_close        =  self.fig2.canvas.mpl_connect('close_event',self.growing_curve_close)
