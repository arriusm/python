...
##### version 2024-11-27 Arno Riffeser arri@usm.lmu.de #####
...
#SN_in   = mag_in  = np.array([1000,1000,1000,1000,1000])  # TOI 5108
...
# mag_in  = 8.92 * np.ones(5)
# mag_in  = np.array([13.8,10.1,9.6,9.5,9.9])  # TOI 5108

texp_in = []
texp_in   = 100.* np.ones(5)   # exposure time per image, sec
# texp_in   = 360.* np.ones(5)   # exposure time per image, sec
# texp_in   = 3600.* np.ones(5)  # exposure time per image, sec
# texp_in   = [    4.32 ,   1.05 ,   1.02 ,   1.58 ,   3.60  ]
# texp_in   = [5.79, 1.09, 0.94, 1.7, 0]
# texp_in   = 1.2* np.ones(5)  # exposure time per image, sec
...
AM       = 1.11 * np.ones(5)   # airmass

ext      = np.array([ 0.56, 0.18, 0.10, 0.08, 0.07])           #  extinction

aperphot = True
#aperphot = False # psf-phot

if tel=='WST 2m' :
  psf         = 4.0 * np.ones(5)  #  seeing/fwhm [arcsec]
  psf         = 10.0 * np.ones(5) #  seeing/fwhm [arcsec]
  psf         = 0.90 * np.ones(5) #  seeing/fwhm [arcsec]
  psf         = 1.00 * np.ones(5) #  seeing/fwhm [arcsec]
...
if tel=='WST 40cm' :
  psf         = 2.2 * np.ones(5) #  seeing/fwhm [arcsec]
  Aper_arcsec = 1.5 * psf  # 3.00 * np.ones(5) # aperture diameter [arcsec]
...
if tel=='WST 40cm' or tel=='WST 40cm defocussed' :
  ...
  q = np.array([ 0.15, 0.35, 0.45, 0.30, 0.0 ])/1.57 # mean throughput in band [q]  20-05-28
...
sig = psf / ( np.sqrt(8.*np.log(2.)) )
# psf_at_AM = np.where(AM>1, psf*AM**0.6, psf)                # seeing/fwhm [arcsec], =psf*AM^0.6
f_gauss = 1.-np.exp(-(Aper_arcsec/2)**2/(2.*sig**2)) # fraction of Gaussian PSF in aperture [N_e,ap / N_e,tot] 
...
print('--------------------------------------------------------------------------------')
print('f_gauss      = [  {:8.2f} ,  {:8.2f} ,  {:8.2f} ,  {:8.2f} ,  {:8.2f}  ]'.format(f_gauss[0],f_gauss[1],f_gauss[2],f_gauss[3],f_gauss[4]))
...
if len(texp_in)==5 and len(mag_in)==5 :
  ...
  print('      Nobj   = [ {:8.1f}  , {:8.1f}  , {:8.1f}  , {:8.1f}  , {:8.1f}   ]'.format(Nobj_psf[0],Nobj_psf[1],Nobj_psf[2],Nobj_psf[3],Nobj_psf[4]))
...
if len(texp_in)==5 and len(SN_in)==5 :
  ...
  if aperphot :
    ...
    mag = mag_psf
  Nobj_sec_LIM_MAG = 10**(-0.4*(mag+AM*ext-ZP))               # object [N_e / sec]
  Nobj_peak = Nobj_sec_LIM_MAG * texp_in * (1-np.exp(-(np.sqrt(1/np.pi)*2*pxsc/2)**2/(2.*sig**2)))  # object  [N_e / pixel]
  Nsky_px  = Nsky_sec * texp_in * fak_px   # nightsky px [N_e / pixel]
  Ngal_px  = Ngal_sec * texp_in * fak_px   # galaxy px [N_e / pixel] 
  #Ntot_px = Nobj_peak + Nsky_px + Ngal_px
  Nobj_psf_LIM_MAG =  Nobj_sec_LIM_MAG * texp_in
  ...
  print('      Npeak  = [  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}   ]'.format(Nobj_peak[0],Nobj_peak[1],Nobj_peak[2],Nobj_peak[3],Nobj_peak[4]))
  print('      Nobj   = [ {:8.1f}  , {:8.1f}  , {:8.1f}  , {:8.1f}  , {:8.1f}   ]'.format(Nobj_psf_SN[0],Nobj_psf_LIM_MAG[1],Nobj_psf_LIM_MAG[2],Nobj_psf_LIM_MAG[3],Nobj_psf_LIM_MAG[4]))
  print('      Nsky   = [  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}   ]'.format(Nsky_px[0],Nsky_px[1],Nsky_px[2],Nsky_px[3],Nsky_px[4]))
...
if len(SN_in)==5 and len(mag_in)==5 :
  ...
  Nobj_psf_TEXP =  Nobj_sec * texp  # object  [N_e]
  #Ntot_px = Nobj_peak + Nsky_px + Ngal_px
  ...
  print('      Npeak  = [  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}   ]'.format(Nobj_peak[0],Nobj_peak[1],Nobj_peak[2],Nobj_peak[3],Nobj_peak[4]))
  print('      Nobj   = [ {:8.1f}  , {:8.1f}  , {:8.1f}  , {:8.1f}  , {:8.1f}   ]'.format(Nobj_psf_TEXP[0],Nobj_psf_TEXP[1],Nobj_psf_TEXP[2],Nobj_psf_TEXP[3],Nobj_psf_TEXP[4]))
  print('      Nsky   = [  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}  ,  {:7.1f}   ]'.format(Nsky_px[0],Nsky_px[1],Nsky_px[2],Nsky_px[3],Nsky_px[4]))
